...
// /* keep buttons here to avoid breaking existing imports */
...
export const Section = styled.section`
  ...
  padding: ${(p) => (p.nopadding ? "0" : "32px 48px 0")};
  margin: 0 auto;
  max-width: 1040px;
  box-sizing: content-box;
  position: relative;
...
export const SectionTitle = styled.h2`
  font-weight: 800;
  font-size: ${(p) => (p.main ? "65px" : "56px")};
  line-height: ${(p) => (p.main ? "72px" : "56px")};
  width: max-content;
  max-width: 100%;
  background: linear-gradient(
    121.57deg,
  ...
  @media ${(p) => p.theme.breakpoints.md} {
    font-size: ${(p) => (p.main ? "56px" : "48px")};
    line-height: ${(p) => (p.main ? "56px" : "48px")};
    margin-bottom: 12px;
    padding: ${(p) => (p.main ? "40px 0 12px" : "0")};
  }

  @media ${(p) => p.theme.breakpoints.sm} {
    font-size: ${(p) => (p.main ? "28px" : "32px")};
    line-height: ${(p) => (p.main ? "32px" : "40px")};
    margin-bottom: 8px;
...
export const ButtonFront = styled.button.withConfig({
  shouldForwardProp: (prop) => prop !== "alt", // allow native `disabled`
})`
  border: none;
  border-radius: 50px;
  color: #fff;
  ...
  box-shadow: ${({ disabled }) =>
    disabled
      ? "inset 0px 2px 1px rgba(46,49,55,.15), inset 0 0 4px rgba(20,20,55,.3)"
      : "none"};
